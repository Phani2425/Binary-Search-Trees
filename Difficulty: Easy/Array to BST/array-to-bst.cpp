...
class Solution {
  public:
   ...
   Node * helperFunction(vector<int>& nums, int start, int end){
       if(start>end){
           return NULL;
       }
       ...
       //create node for that mid
       Node * temp = new Node(nums[mid]);
       
       temp->left = helperFunction(nums,start,mid-1);
       temp->right =helperFunction(nums,mid+1,end);
